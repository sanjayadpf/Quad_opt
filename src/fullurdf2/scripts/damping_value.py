...
import rospy
from fullurdf2.srv import SetDamping, SetDampingRequest
import time
...
def change_damping():
    ...
    rospy.wait_for_service('/gazebo/set_joint_damping_fullurdf2')
    rospy.loginfo("Service /gazebo/set_joint_damping_fullurdf2 found")
    set_damping = rospy.ServiceProxy('/gazebo/set_joint_damping_fullurdf2', SetDamping)
    ...
    for joint_index in range(2, 51):
        ...
        rospy.loginfo(f"Calling /gazebo/set_joint_damping_fullurdf2 service for {joint_name} with damping {damping_value}")
