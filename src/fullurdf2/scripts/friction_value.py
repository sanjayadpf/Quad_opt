...
import rospy
from fullurdf2.srv import SetFriction, SetFrictionRequest
import time
...
def change_friction():
    ...
    rospy.wait_for_service('/gazebo/set_joint_friction_fullurdf2')
    rospy.loginfo("Service /gazebo/set_joint_friction_fullurdf2 found")
    set_friction = rospy.ServiceProxy('/gazebo/set_joint_friction_fullurdf2', SetFriction)
    ...
    for joint_index in range(2, 51):
        ...
        rospy.loginfo(f"Calling /gazebo/set_joint_friction_fullurdf2 service for {joint_name} with friction {friction_value}")
