...
import rospy
from softactuator.srv import SetFriction, SetFrictionRequest
import time
...
def change_friction():
    ...
    rospy.wait_for_service('/gazebo/set_joint_friction_softactuator')
    rospy.loginfo("Service /gazebo/set_joint_friction_softactuator found")
    set_friction = rospy.ServiceProxy('/gazebo/set_joint_friction_softactuator', SetFriction)
    ...
    for joint_index in range(2, 12):
        ...
        rospy.loginfo(f"Calling /gazebo/set_joint_friction_softactuator service for {joint_name} with friction {friction_value}")
