...
def change_stiffness_and_damping():
    ...
    for stiffness in [round(x * 0.1, 1) for x in range(10, 51)]:
        for joint_index in range(2, 12):
            ...
            rospy.loginfo(f"Calling /gazebo/set_joint_stiffness_softactuator service for {joint_name} with stiffness {stiffness}")
            ...
            rospy.loginfo(f"Calling /gazebo/set_joint_damping_softactuator service for {joint_name} with damping {damping_req.damping}")
