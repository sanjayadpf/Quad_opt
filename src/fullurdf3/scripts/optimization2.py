...
if __name__ == '__main__':
    try:
        ...
        search_space = [
            Real(0.10, 0.35, name='scaled_torque'),  # Torque 1 bounds
            Real(0.10, 0.35, name='back_torque'),    # Torque 2 bounds
            Real(0.10, 0.35, name='spine_torque')             # Rate as an integer between 5 and 50
        ]

        # Perform Bayesian Optimization using Gaussian Process
        result = gp_minimize(
            func=objective_function,          # Objective function to minimize
            dimensions=search_space,          # Search space for both torques and rate  
            n_calls=100,                      # Number of function evaluations
            acq_func="PI",                    # Expected improvement acquisition function EI/PI/LCB
            random_state=42                   # For reproducibility
        )

        # Extract the optimal torques and rate, along with the corresponding maximum distance
        optimal_scaled_torque = round(result.x[0], 3)  # Optimal scaled_torque
        optimal_back_torque = round(result.x[1], 3)    # Optimal back_torque
        optimal_spine_torque = round(result.x[2],3)      # Optimal spine_torque
        max_distance = round(-result.fun, 4)           # Result.fun is the minimized value (negative distance)

        print(f"Optimal scaled_torque: {optimal_scaled_torque}, Optimal back_torque: {optimal_back_torque}, Optimal spine_torque: {optimal_spine_torque }, Maximum distance: {max_distance}")
