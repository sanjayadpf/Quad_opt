...
def spine_activate(diffleg_torque, rate):
    ...
    rate = rospy.Rate(rate)  # Control the rate of execution
    ...
    return -1 * position.y  # Return the negative y position (or other relevant metric)
...
if __name__ == '__main__':
    try:
        ...
        while True:
             #reset simulation
            reset_simulation()
            dist=spine_activate(diffleg_torque=[0.255,0.255], rate=8)
        #test result
        #dist=spine_activate(diffleg_torque=[0.25,0.25], rate=8)
            print(dist)
